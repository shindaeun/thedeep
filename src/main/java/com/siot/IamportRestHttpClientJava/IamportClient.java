...
public IamportResponse<Payments> paymentAll(String page) throws Exception {
		
		String token = this.getToken();
		
		if(token != null){
			String path = "/payments/status/all?page="+page;
			String response = this.getRequest(path, token);
			System.out.println("response : "+response);
			
			Type listType = new TypeToken<IamportResponse<Payments>>(){}.getType();
			IamportResponse<Payments> payment = gson.fromJson(response, listType);
			return payment;
		}
